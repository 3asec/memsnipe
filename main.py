...
def get_price_sell(token_address_checksum, amount):
    ...
    data = response.json()
    return float(data["quote"]["output"]["amount"])
...
def sell_tx(token_address_checksum):
    ...
    ahaaaa = get_price_sell(token_address, amounts)
    while(ahaaaa <= amount_tp or ahaaaa >= amount_cl):
        time.sleep(1)
        print("Waiting for Price to be in Range....")
        ahaaaa = get_price_sell(token_address, amounts)
